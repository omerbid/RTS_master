...
| **Zip – PopulationCost** | שדה ב־CSV. אצלנו יש Upkeep, RecruitCostMoney, ConvertPopulationCost ב־FUnitRow. | PopulationCost ב־zip יכול להתאים ל־Upkeep או ל־ConvertPopulationCost. | להחליט: אם רוצים "מחיר אוכלוסייה" גנרי – אפשר להוסיף שדה או למפות ל־ConvertPopulationCost. |
...
## 3. סיכום קונפליקטים לאישור איתך

1. **תפקידי יחידות (Role):**  
   - **אפשרות א:** להרחיב את `EUnitRole` בערכים מהטבלה (Frontline, Ranged, AntiLarge, EliteRanged, Flanker, Elite, Assault, Skirmisher, EliteFrontline, Support) – חלקם כבר קיימים (Ranged→RangedLight, Support).  
   - **אפשרות ב:** למפות את 12 היחידות מה־zip ל־Roles קיימים בלי לשנות את ה־enum.  
   - **לאשר:** איזו אפשרות מעדיף?

2. **PopulationCost:**  
   - האם להגדיר ש־PopulationCost ב־zip = ConvertPopulationCost אצלנו, או להוסיף שדה נפרד PopulationCost ב־FUnitRow?

3. **ייבוא טבלת האיזון:**  
   - האם לייבא את unit_balance_table.csv ל־DT_Units_MVP (כ־rows חדשים/עדכון) – עם מיפוי Role ו־AttackCooldown↔AttackSpeed?

אחרי שתאשר את שלושת הנקודות האלו, אפשר לפרט משימות קונקרטיות (שינוי enum / שדות / ייבוא נתונים).
...
| **אופציונלי לאחר אישורך** | ייבוא/הרחבת DT_Units_MVP מטבלת האיזון (zip); הרחבת/מיפוי EUnitRole; שדה PopulationCost אם נבחר. |
...
### אם רוצים "סגירת מעגל" עם החבילות

- **לאשר** את שלושת הנקודות בסעיף 3 (Role, PopulationCost, ייבוא CSV).
- **לבצע** (אם תאשר): עדכון/הרחבת EUnitRow (או EUnitRole), ייבוא unit_balance_table ל־DT_Units_MVP, עדכון P0_ALIGNMENT אם נוספו שדות.

**סיכום:** אחרי העדכון על החבילות – **אין חסר קריטי ל־P0–P3**. יש אופציונלי (טבלת איזון + מיפוי תפקידים) שכדאי לאשר איתך לפני יישום.
